export const configAxios = {
    headers: {
        Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6OCwiaWF0IjoxNzA0OTc1OTIwLCJleHAiOjE3MDc1Njc5MjB9.o1LckTvU16MOeplOVGvrgSaWI0cVgto5sFvKVe7sQ4k',
    },
};

export const baseUrl = 'https://ideacao-backend-8ea0b764c21a.herokuapp.com/api/';

export const baseUrlServicos = baseUrl + 'valentim-servicos/';
export const baseUrlClientes = baseUrl + 'valentim-clientes/';
export const baseUrlAgendamentos = baseUrl + 'valentim-agendamentos/';